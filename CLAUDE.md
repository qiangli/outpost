...
- **`internal/agent/git/`** — pure-Go git client wrapping `go-git/v5`, exposed as `outpost git …` (clone, init, add, commit, status, log, diff, branch, checkout, push, pull, fetch, remote, show, merge, merge-base, rev-list, config, tag, reset, rm, ls-files, blame, grep, rev-parse). The load-bearing case is Windows-without-system-git: outpost ships a self-contained git so users don't have to wire up a real `git` binary + credential helper on every machine. Even local-path remotes stay pure Go — `transport.go` swaps go-git's stock "file" transport (which shells out to `git-upload-pack`) for the in-process server, with a haves-filter wrapper so fetching into an ahead/diverged clone doesn't trip go-git's server-side "object not found". `pull` is a hand-rolled fetch + fast-forward (NOT go-git's `Worktree.Pull`, which integrates the remote's HEAD instead of the current branch's upstream, errors when the local branch is merely ahead, and refuses any unstaged change); it preserves non-conflicting local changes via a manual tree-diff apply (`merge.go:ffUpdate`) and reports git-style errors when local edits would be overwritten. Diverged histories are an error by design — anything needing conflict resolution (rebase, stash, cherry-pick, apply, submodules, worktrees, reflog, bisect) is intentionally out of scope: a half-applied replay with no host-git fallback is worse than no support. HTTPS auth supports explicit `--username/--password` and falls back to `$GITHUB_TOKEN` / `$GIT_TOKEN` as an oauth2-style basic-auth password. SSH auth takes `--ssh-key [--ssh-key-pass]`. No MCP surface — classic `git` is CLI-only, so `outpost git` matches. Cobra wiring in `cmd/outpost/git.go` + `cmd/outpost/git_verbs.go`.
