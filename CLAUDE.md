...
go run ./cmd/outpost upgrade [--local PATH | --from URL | --direct]   # operator-driven self-upgrade (--direct = latest GitHub release)
...
go run ./cmd/outpost-vk -kubeconfig ~/.kube/config -node <name>  # standalone virtual-kubelet PoC (vknode)
...
- `GET /desktop` — WebSocket ↔ TCP VNC relay (`--vnc-addr`, default `127.0.0.1:5900`). The relay terminates RFB auth itself from the browser's first JSON frame `{user,password}`: Apple ARD (type 30) with an OS user, VNC Authentication (type 2, DES) or None with an empty user — so it fronts macOS Screen Sharing and the Windows/Linux servers alike (`vncauth.go`). Also published to mesh peers as mesh service `desktop` (`cmd/outpost/desktop_wiring.go`: a loopback listener serving `/desktop` ONLY — never the main engine, whose `/shell` has no auth of its own) so a peer's bashy app opens this screen over the direct link.
...
- **OS path (`AuthURL == ""`)**: the submitted username MUST refer to the agent's own running OS user — compared via `hostauth.SameUser`, which accepts the exact local form or the bare account name case-insensitively (so `alice` matches Windows' SAM-compatible `MACHINE\alice`; authentication always runs against the canonical form, so the widened match never changes who gets verified); `hostauth.Authenticator` verifies the password via dscl (macOS) / `unix_chkpwd`+`/etc/shadow` (Linux, **cgo-free**) / LogonUserW (Windows). The platform implementations live in `internal/agent/hostauth/hostauth_{darwin,linux,windows}.go` split by build tags. The Linux path is deliberately pure Go (no cgo PAM): release binaries cross-compile with `CGO_ENABLED=0`, so a cgo authenticator would never link — it shells out to the setuid `unix_chkpwd` helper and falls back to reading `/etc/shadow` + verifying the crypt(3) hash (the fallback covers Ubuntu 26.04 / PAM 1.7.0, whose `unix_chkpwd` rejects valid passwords). LDAP/SSSD/AD accounts that aren't in local `/etc/shadow` need `AUTH_URL` mode. Role defaults to `admin` (whoever can prove the OS password owns the box). If `AdminUsers` is non-empty it acts as an allowlist over the cloud-trusted `X-Periscope-User` header; missing entries get downgraded to `user`.
...
- `ssh` — same listener+WS-bridge shape as `tcp`, but the bridge targets the remote outpost's **built-in `/ssh` endpoint** (the in-process `golang.org/x/crypto/ssh` server) directly, dialing `wss://<cloudbox>/matrix/h/<host>/ssh`. No `AppConfig` on the remote required. The `Name` field is ignored. Elevate flow uses the per-builtin cloudbox endpoint `POST /matrix/h/<host>/elev/ssh` (with `elev` as a literal segment — *not* `/matrix/h/<host>/ssh/elevate`, even though the 410 handler that replaced the legacy host-wide `/matrix/h/<host>/elevate` hints at the suffix form; the real route uses `/elev/` to avoid colliding with gin's catch-all on `/matrix/h/:host/app/:name`). Pinger hits `/matrix/h/<host>/elev/ssh/ping`. Use case: `ssh -p <local_port> alice@127.0.0.1` from a roaming host-a to reach a paired outpost's built-in SSH without needing a host-OS sshd port mapping.
...
A third built-in, `sandbox` (`BuiltinSandbox`, gated on `SandboxEnabled`, default off), registers a SEPARATE app on the same podman socket `DetectPodman()` finds — but wraps the proxy with a **filter** so it is safe to expose to a thin client or an untrusted tenant, unlike the raw `/app/podman/` passthrough (which stays admin-only for trusted self-use). The package mirrors `internal/agent/ollama/`:

- `filter.go` — the security core. An `http` middleware that vets `POST .../containers/create` and `.../exec` bodies in BOTH wire shapes (docker-compat PascalCase `HostConfig`, and libpod lowercase SpecGenerator — ycode's gateway drives the latter). It denies `privileged`, host network/PID/IPC/UTS/user/cgroup namespaces, host bind mounts outside `ScratchHostPrefix`, added capabilities, device passthrough, and confinement-disabling `security-opt`/`selinux_opt`; it injects/clamps per-request memory/CPU/pids caps. Default-deny: an unparseable create body is rejected, never forwarded.
- `policy.go` — `Policy{MaxMemoryBytes, NanoCPUs, PidsLimit, MaxContainers, AllowedImages, ScratchHostPrefix}`, built in `main.go` from the `sandbox_*` FileConfig fields. Escape knobs are NOT tunable — the mount always strips them.
- `service.go` / `counter.go` / `types.go` — glue + in-flight counter + the `CapacityReport` wire shape. `WrapProxy` runs `filter → counter → proxy`; `CapacityHandler` serves `/app/sandbox/_pool/capacity`.
- `prewarm.go` — a self-contained libpod image client (stdlib only, no vknode dependency) that pre-pulls the runnable images (`sandbox_prewarm_images`, defaulting to the concrete `sandbox_allowed_images` entries) so a remote create+start skips the image-pull cost — the dominant cold-start latency. A transparent pre-warmed *container* pool can't work at the raw-docker layer (the caller picks image/cmd/env at create time), so pre-pulling is the correct Phase-A warm. Started under the errgroup in `main.go` (like the ollama watcher); readiness surfaces in `CapacityReport.WarmImages`.
...
### Intra-home distributed inference / sharding (`internal/agent/clusterllm/`)

The LLM pool above load-balances across single-box daemons — it can route a model to a box that has it, but it can't run a model **bigger than any one box's VRAM**. Intra-home *sharding* lets several boxes on one LAN cooperatively serve a model none could hold alone (e.g. a 70B across 2–3 consumer boxes). The principle is **wrap a runtime, don't build one**: the actual tensor/pipeline split is done by an external distributed-inference backend (GPUStack multi-node vLLM, Prima.cpp, llama.cpp `--rpc`, …); the outpost just **detects it, measures its aggregate capacity, and advertises the home to cloudbox as one logical big-VRAM host**.

- **Detection + aggregation (`clusterllm/{detect,capacity}.go`)** — gated on `fc.ClusterLLMOn()` (non-empty `cluster_llm_endpoint`). `Detector.Info(ctx)` probes the backend's reachability and, when a `cluster_llm_api_key` is set, aggregates total VRAM + worker count off the backend's management API (GPUStack `/v2|/v1/gpu-devices` today; `BackendGPUStack` is an extensible constant). Inert-on-failure: any probe error degrades to `MaxModelBytes=0`, leaving the cloudbox-side size filter off.
- **Advertisement** — `cmd/outpost/main.go`'s `clusterSourceAdapter` bridges the detector to the watcher's `ClusterSource`; the registry push then carries an optional `Cluster{max_model_bytes, member_count, backend}` substruct (`types.go`), folded into the content hash (`content_hash.go` `clusterHashTag`) so a membership change re-pushes. The cloud side reads `max_model_bytes` to size-gate routing — a too-big model is routed only to homes whose cluster can hold it — and surfaces the cluster on its public model list (the `x_cluster` field on `/v1/models`).

**The serving path — two backend modes:**

1. **Ollama-API backend (zero extra code, recommended).** If the cluster head speaks the Ollama API (a distributed-Ollama / `--rpc` head, or any Ollama-compatible front), point the outpost's Ollama target at it: `OLLAMA_HOST=http://<cluster-head>:<port>`. Then **inventory and serving both flow to the cluster** through the existing `/app/ollama/*` proxy + watcher — nothing else changes — while `cluster_llm_endpoint` (the management API) supplies the VRAM aggregation. This is the closed, working path.
2. **OpenAI-only backend (e.g. GPUStack).** GPUStack serves OpenAI (`/v1-openai/...`) but **not** Ollama's `/api/tags`/`/api/show`, so the Ollama-native watcher can't inventory it directly. Closing this path needs an OpenAI↔Ollama inventory+serving shim in the outpost — a real change that must be validated against a live backend; **tracked as a follow-on**, not yet wired. Until then GPUStack is usable for VRAM advertisement + cloudbox-side selection but a home wanting GPUStack serving uses mode 1 (front GPUStack with an Ollama-compatible endpoint) or runs an Ollama-API cluster backend.
...
- **`upgrade.GitHubSource`** (`github.go`) — the **direct-from-GitHub** resolver, the fallback upgrade authority for the two cases the cloudbox push/pull path structurally can't serve: an **unpaired** host (no `access_token`, so no `/api/v1/fleet/target` to poll) and the `outpost upgrade --direct` operator escape hatch on a paired host. `Resolve(ctx)` GETs `api.github.com/repos/<repo>/releases/latest`, picks the `outpost-<tag>-<goos>-<goarch>[.exe]` asset, reads the matching `.sha256` sidecar, dereferences the tag ref to its commit sha (handling annotated tags), and returns the **same `Envelope` shape** cloudbox's release webhook produces — so everything downstream (StageFromURL sha256 verify → Probe commit-match → RetainPrevious → atomic swap) is byte-for-byte the same path. The only thing that shifts is the artifact owner: GitHub Releases over HTTPS instead of cloudbox. Honors `$GITHUB_TOKEN`/`$GIT_TOKEN` to lift the 60-req/hr unauthenticated API limit. CLI wiring is in `cmd/outpost/upgrade.go`: `outpost upgrade` with **no** source flag defaults to `--direct` on an unpaired host and **refuses** on a paired host (directs the operator to the fleet, with `--direct` as the explicit override). Deliberately NOT wired into a background poller — paired hosts' automatic upgrades stay governed by the fleet (canary→fleet, `update_mode`, `min_from`), and an unpaired host self-upgrading silently could surprise an operator who pinned a build on purpose; direct resolution is always an explicit, interactive action.

Trust model: HTTPS-to-cloudbox + sha256-in-envelope + cloudbox-as-artifact-owner (or, for `--direct`, HTTPS-to-GitHub + sha256-from-sidecar + GitHub-Releases-as-artifact-owner). An `ArtifactVerifier` hook is reserved for future signed-manifest validation but defaults to the no-op probe — the daemon will refuse a candidate that doesn't self-report the envelope's commit, which is the load-bearing check today.
...
### Cluster join — k3s-agent + virtual-kubelet (`internal/agent/{runtime,vknode,userkube}`, `cmd/outpost-{vk,cni}`)

The cluster story is the newest and most cross-cutting subsystem; it has three independent halves that share a `FileConfig.Cluster.{enabled, runtimes, api_url, token, ca, node_name}` config block. `runtimes.agent` enables one real k3s agent and `runtimes.virtual` selects any combination of `vk-native`, `vk-podman`, and `vk-ollama`; all selected runtimes run concurrently, with at most one node per backend.

- **Half A — k3s-agent in a podman container (`internal/agent/runtime/`).** `runtime.Up(ctx, opts)` supervises a podman/docker container that hosts this outpost's kubelet + containerd. The container registers one agent node and is independent of any selected virtual nodes. Idempotent — repeated `Up` with the same name reattaches. `ErrPodmanNotFound` surfaces a clear install-Docker/Podman message. Enable it with `outpost builtins set --cluster-agent on`. Image is built once via `outpost cluster init` / `build-runtime` or pulled from a registry. See `docs/cluster-gpu.md` for the NVIDIA driver + container-toolkit prerequisites needed on Linux GPU hosts.
- **Half B — virtual-kubelet provider (`internal/agent/vknode/`).** Each selected virtual backend registers its own node, named `<node-name>-<backend>`, while sharing the registered physical host identity and cloud-issued credentials. Three layers — Layer 1: hand-rolled HTTP-over-local-socket client for the libpod REST API (deliberately avoids `containers/podman/v5/pkg/bindings` because it pulls in `containers/storage` (cgo) and would break cross-compile). Its `DialContext` comes from `localsock.DialFunc`, so it reaches a unix socket OR a Windows named pipe. Layer 2: `translate.go` converts `corev1.Pod` → libpod `SpecGenerator`, stamping `outpost.io/managed=true` + namespace/name/uid labels for reconciliation and `podman ps` legibility; Layer 3: `provider.go` / `node.go` implement virtual-kubelet's `PodLifecycleHandler` and `NodeProvider`. `cmd/outpost-vk/` is a standalone PoC runner. `userkube` consumes vknode's kubeconfig fetcher for the operator-facing rendering.
